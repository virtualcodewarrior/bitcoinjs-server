...
Block.prototype.checkProofOfWork = function checkProofOfWork() {
  if (this.cfg.network.altChain == true && this.version & this.cfg.network.auxPOWFlag != 0) {
    if (this.getAuxChainID() != this.cfg.auxPOWChain)
      throw new VerificationError('Auxiliary Chain ID doesn\'t match');
    }

    // This is an auxiliary proof-of-work block
    console.log('AuxChain: '+this.getAuxChainID());
    console.dir(this);
  }

  var target = Util.decodeDiffBits(this.bits);

  var thisHash = Util.cloneBuffer(this.hash);
  thisHash.reverse();
