...
Block.prototype.checkProofOfWork = function checkProofOfWork() {
  ...
  if (this.isAuxPOW()) {
    ...
    if (this.aux.parent.getHash().compare(this.aux.parent_hash) != 0) {
      throw new VerificationError('Auxiliary POW parent block header is not correct');
    }
...
Block.prototype.checkAuxCoinbase = function checkAuxCoinbase() {
  ...
  var header = new Buffer('FABE6d6d', 'hex');
  var head_index = script.indexOf(header);
  if (head_index < 0) {
    throw new VerificationError('AuxPOW coinbase script header not found');
  }
  if (script.indexOf(header, head_index+1) >= 0) {
    throw new VerificationError('Multiple AuxPOW script headers found in coinbase script');
  }

  var script_data = {};
  script_data.hash = script.slice(head_index+4, head_index+4+32);
  script_data.size = script.readUInt32LE(head_index+4+32);
  script_data.nonce = script.readUInt32LE(head_index+4+32+4);

  var block_hash = Util.cloneBuffer(this.getHash());
  block_hash.reverse();
  if (block_hash.compare(script_data.hash) != 0) {
    throw new VerificationError('AuxPOW block header not found in coinbase script');
  }
