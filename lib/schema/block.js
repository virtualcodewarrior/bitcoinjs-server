...
var MerkleLink = require('./merklelink').MerkleLink;
...
function Block (data)
{
  ...
  if (this.cfg.network.altChain === true) {
    ...
    this.coinbase = data.coinbase || new Transaction();
    this.aux_branches = data.aux_branches || [];
    this.aux_index = data.aux_index || 0;
    this.aux_parent = new Block(data.aux_parent);
    this.merkle_link = new MerkleLink(data.merkle_link, this);
  }
};
...
Block.prototype.checkProofOfWork = function checkProofOfWork() {
  ...
  if (this.isAuxPOW()) {
    ...
    if (this.aux_parent.getHash().compare(this.merkle_link.block_hash) != 0) {
      throw new VerificationError('Auxiliary POW parent block header is not correct');
    }
    var thisHash = Util.cloneBuffer(this.aux_parent.getHash());
  } else {
