...
var COINBASE_OP = exports.COINBASE_OP = Buffer.concat([Util.NULL_HASH, Util.decodeHex("FFFFFFFF")], 36);
    ...
    function verifyInputsResults(err, results) {
      ...
      for (var i = 0, l = results.length; i < l; i++) {
        if (!results[i]) {
          ...
          logger.scrdbg('Script for input '+i+' evaluated to false');
          logger.scrdbg('|- scriptSig '+self.ins[i].getScript().toString());
          logger.scrdbg('`- scriptPubKey '+txout.getScript().toString());
