...
var isExiting = false; // Flag to allow a controlled shutdown
process.on('SIGINT', function() {
  if (isExiting) {
    // Force shutdown with back-to-back interrupts
    logger.error('Forced shutdown requested; killing blockchain download ungracefully!');
    process.exit(0);
  }
  isExiting = true;
  logger.warn('Shutdown requested... Use '+'CTRL-C'.bold+' to '+'KILL'.bold.red+' immediately');
});
...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  // TODO: This should be in Settings (network settings)
  ...
  function isTestnet() {
    return self.cfg.network.type == 'testnet';
  };
  ...
  var getTargetTimespan = this.getTargetTimespan =
  function getTargetTimespan() {
    return targetTimespan;
  };

  var getTargetSpacing = this.getTargetSpacing =
  function getTargetSpacing() {
    return targetSpacing;
  };

  /**
   * Insert ourselves between the database fetch and the given callback
   * Such that we can attach our own settings to the returned block
   */
  var getBlock = this.getBlock = function getBlock(method, callback, param) {
    storage[method].call(storage, param, function(err, block) {
      if (err) {
        callback(err);
        return;
      }
      if ('undefined' != typeof block) block.cfg = self.cfg;
      callback(null, block);
    });
  };
  ...
  function getBlockByPrev(block, callback) {
    getBlock('getBlockByPrev', callback, block);
  }
  ...
  var processBlock = this.processBlock = function processBlock(bw) {
    ...
    Step(
      ...
      function connectStep(err) {
        if (err) throw err;

        connectBlock(bw, this);
      },
      function verifyConnectionStep(err) {
        if (err) throw err;
      ...
      function startTransactionStep(err) {
        if (err) throw err;

        storage.startTransaction(this);
      },
      function reorganizeStep(err) {
        if (err) throw err;
      ...
      function saveTransactionsStep(err) {
        if (err) throw err;

        self.saveTransactions(bw.block, bw.txs, this);
      },
      function saveBlockStep(err) {
        if (err) throw err;

        self.saveBlock(bw, this);
      },
      function connectTransactionsStep(err) {
        if (err) throw err;
      ...
      function endTransactionStep(err) {
        if (err) throw err;

        storage.endTransaction(this);
      },
      function queueDependentsStep(err) {
        if (err) throw err;
      ...
      function finalizeStep(err) {
        ...
        if (incomingBlockQueue.length) {
          if (isExiting && incomingBlockQueue.length % 10 == 0) logger.info('Finishing processing '+incomingBlockQueue.length+' blocks before shutting down');
          ...
          isProcessing = false;
          if (isExiting) {
            self.storage.disconnect(function() {
              logger.info('Blockchain queue emptied; shutting down');
              process.exit(0);
            });
          } else {
  ...
  function connectBlock(bw, callback) {
    ...
    // Does this block connect to another orphan block?
      ...
      logger.bchdbg('Connecting block '+
                    Util.formatHashAlt(bw.block.hash)+' '+
  ...
  function connectToMainChain(bw, callback)
  {
    ...
    logger.bchdbg('Adding block '+Util.formatHashAlt(bw.block.hash));
  ...
  function connectToSideChain(bw, parent, callback)
  {
    ...
    logger.info('Adding block '+Util.formatHashAlt(bw.block.hash)+
                ' on side chain');
      ...
      function cacheTxInputs() {
        ...
        bw.txs.forEach(function (tx, i) {
          ...
          tx.cacheInputs(self, localTx, true, function (err, txCache) {
            ...
            var isCoinbase = i === 0 && tx.isCoinBase();
            if (isCoinbase) {
              // We won't verify coinbase transactions
              callback(null);
            } else if (self.cfg.verifyScripts && self.isPastCheckpoints()) {
              tx.verify(txCache, self, function (err) {
                // Prepend tx id for verification errors for easier debugging
                if (err instanceof VerificationError) {
                  err.message = "Tx "+Util.formatHashAlt(tx.hash)+": "+
                    err.message;
                }
              ...
              // We won't verify because verification of scripts is turned off
              callback(null);
            }
          });
        });
  ...
  function connectTransactions(block, txs, callback)
  {
        ...
        self.getBlockByHash(bOld.prev_hash, function (err, bOldParent) {
          ...
          if (!bOldParent) {
            logger.error("Active branch was disconnected, cannot find "+
                         Util.formatHashAlt(bOld.prev_hash));
        ...
        self.getBlockByHash(bNew.prev_hash, function (err, bNewParent) {
          ...
          if (!bNewParent) {
            logger.error("New branch was disconnected, cannot find "+
                         Util.formatHashAlt(bNew.prev_hash));
  ...
  this.reorganize = function reorganize(oldTopBlock, newTopBlock, callback) {
    logger.info('Reorganize (old head: '+Util.formatHashAlt(oldTopBlock.hash)+
                ', new head: '+Util.formatHashAlt(newTopBlock.hash)+')');
    ...
    this.findFork(oldTopBlock, newTopBlock, function (err, toDisconnect, toConnect) {
      ...
      logger.bchdbg('Found common root at '+Util.formatHashAlt(toConnect[0].prev_hash));
...
BlockWrapper.prototype.callback = function callback(err)
{
  ...
  // callbacks or otherwise interfere if we're not in a consistent state.)
