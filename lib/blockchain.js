...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  this.close = function close(callback) {
    ...
    if (isProcessing) {
      logger.bchdbg('Waiting for blockchain queue to parse...');
      return; // Wait for queue to empty
    }
    
    Step(
      function closeDB() {
        logger.bchdbg('Closing blockchain data store...');
        self.storage.disconnect(this);
      },
      function cleanup() {
        logger.bchdbg('Blockchain ready for shutdown');
        return null;
      },
      callback
    );
    ...
    Step(
      ...
      function finalizeStep(err) {
        ...
        if (incomingBlockQueue.length) {
          if (isExiting !== false && incomingBlockQueue.length % 50 == 0) logger.info('Finishing processing '+incomingBlockQueue.length+' blocks before shutting down');
