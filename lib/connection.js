...
Connection.prototype.parseMessage = function (command, payload) {
  ...
  case 'block':
    ...
    if (this.node.cfg.network.altChain === true && (data.version & this.node.cfg.network.auxPOWFlag) != 0) {
      ...
      data.coinbase = Connection.parseTx(parser);

      data.merkle_link = {};
      data.merkle_link.block_hash = parser.buffer(32);
      var merkleCount = Connection.parseVarInt(parser);
      data.merkle_link.branches = [];
      for (i = 0; i < merkleCount; i++) {
        data.merkle_link.branches.push(parser.buffer(32));
      }
      data.merkle_link.index = parser.word32le();

      var auxCount = Connection.parseVarInt(parser);
      data.aux_branches = [];
      for (i = 0; i < auxCount; i++) {
        data.aux_branches.push(parser.buffer(32));
      }
      data.aux_index = parser.word32le();
