...
Connection.prototype.parseMessage = function (command, payload) {
  ...
  case 'block':
    ...
    if (this.node.cfg.network.altChain === true && (data.version & this.node.cfg.network.auxPOWFlag) != 0) {
      // This is an Auxiliary Proof of Work block
      data.aux_coinbase = Connection.parseTx(parser);

      data.aux_parent_hash = parser.buffer(32);
      var merkleCount = Connection.parseVarInt(parser);
      data.aux_coinbase_branch = [];
      for (i = 0; i < merkleCount; i++) {
        data.aux_coinbase_branch.push(parser.buffer(32));
      }
      data.aux_coinbase_branch_mask = parser.word32le();

      var auxCount = Connection.parseVarInt(parser);
      data.aux_blockchain_branch = [];
      for (i = 0; i < auxCount; i++) {
        data.aux_blockchain_branch.push(parser.buffer(32));
      }
      data.aux_blockchain_mask = parser.word32le();

      data.aux_parent = {};
      data.aux_parent.version = parser.word32le();
      data.aux_parent.prev_hash = parser.buffer(32);
      data.aux_parent.merkle_root = parser.buffer(32);
      data.aux_parent.timestamp = parser.word32le();
      data.aux_parent.bits = parser.word32le();
      data.aux_parent.nonce = parser.word32le();
    }
