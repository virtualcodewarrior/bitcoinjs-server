...
var LevelDownStorage = exports.LevelDownStorage = exports.Storage =
    function LevelDownStorage(uri) {
        ...
        var connect = this.connect = function connect(callback) {
            ...
            logger.info("Initializing LevelDB (LevelDOWN; "+uri+")");
            ...
            function getTransactionByHash(hash, callback) {
                hMain.get(hash, defaultGetOpts, function (err, data) {
                    ...
                    if (data) {
                        ...
                        var reqHash = Util.formatHashFull(hash);
                        var rcvHash = Util.formatHashFull(data.getHash());
                        
                        if (reqHash !== rcvHash) {
                          // TODO lookup the block of the requested transaction, and re-request its download
                          throw new Error('Transaction '+reqHash+' is not saved properly in the database; data hashes to '+rcvHash+' instead');
                        }
                    }
                    callback(null, data);
                });
        ...
        var getTransactionsByHashes = this.getTransactionsByHashes =
            function getTransactionsByHashes(hashes, callback) {
                Step(
                    ...
                    function (err, result) {
                        if (err && !keyNotFound(err)) throw err;
                        var txs = [];
            ...
            function getBlockByHash(hash, callback) {
                hMain.get(hash, defaultGetOpts, function getBlockByHashCallback(err, data) {
                    ...
                    if (data) {
                        ...
                        if (reqHash !== rcvHash) {
                          // TODO re-request the block
                          throw new Error('Block '+reqHash+' is not saved properly in the database; data hashes to '+rcvHash+' instead');
                        }
                    }
        ...
        var getTopBlock = this.getTopBlock =
            function getTopBlock(callback) {
                getBlockByHeight(metadata.chainHeight, function(err, block) {
                  if (err) throw err;
                  if (block.active || metadata.chainHeight == 0) {
                    callback(null, block);
                    return;
                  }
                  metadata.chainHeight--;
                  logger.bchdbg('Top block inactive; rolling back to '+metadata.chainHeight+'...');
                  if (metadata.chainHeight < 0) {
                    callback(new Error("Block chain empty or corrupted"));
                    return;
                  }
                  setMeta('chainHeight', metadata.chainHeight, function() {
