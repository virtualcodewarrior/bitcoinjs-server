...
var Node = function Node(cfg) {
  ...
  process.once('SIGINT', function() {
    ...
    process.once('SIGINT', function() {
      ...
      logger.error('Forced shutdown requested; killing ungracefully!');
      process.exit(0);
    });
    
    Step(
      function closeBlockchain() {
        self.blockChain.close(this);
      },
  ...
  memwatch.on('stats', function(stats) {
    ...
    function asMB(num) {
      var MB = num / 1232896;
      var MB = parseInt(MB * 1000)/1000; // Round to 3 places
      return MB+' MB';
    }
    
    var prefix = "\u251c\u2500";
    var tail = "\u2514\u2500";
    logger.info('Memwatch status triggered: GC number '+stats.num_full_gc);
    logger.info(prefix+' Current Usage: '+asMB(process.memoryUsage().rss));
    logger.info(prefix+' Current Base: '+asMB(stats.current_base));
    logger.info(prefix+' Estimated Base: '+asMB(stats.estimated_base));
    logger.info(prefix+' Min: '+asMB(stats.min));
    logger.info(prefix+' Max: '+asMB(stats.max));
    logger.info(tail+' Chain height: '+self.blockChain.getHeight());
  });
...
Node.prototype.handleBlock = function (e) {
  ...
  var blockData = {};
  for (var attr in e.message) {
    if (e.message.hasOwnProperty(attr)) blockData[attr] = e.message[attr]; // Clone message
  }
  blockData.txs = []; // We don't need the transactions immediately
  var block = this.blockChain.makeBlockObject(blockData);

  block.hash = block.calcHash();
  block.size = e.message.size;
  block.sourcePeer = e.peer;
...
Node.prototype.handleBlockAddCallback = function (err) {
  ...
  if (err) {
    logger.error("Error adding block "+
                 Util.formatHashAlt(block.hash)+" from peer "+block.sourcePeer.toString()+": "+
